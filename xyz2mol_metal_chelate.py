...
def get_UA(maxValence_list, valence_list):
    """
    Returns: UA, DU
    UA is list of indices of atoms for which maxValence > valence (which is no of neighbours?, i.e. valence_list is AC_valence)
    DU is the "left over" valence, i.e. maxValence - valence. Say for carbon, 4-2 = 2. (Maybe that means double/triple bonds exist?)
    """
    UA = []
    DU = []
...
def BO_is_OK(BO, AC, charge, DU, atomic_valence_electrons, atoms, valences,
    ...
    # print("here BO and AC different", BO, AC)
...
def get_bonds(UA, AC):
    """
    Based on AC matrix (atomic connectivity) and given indices of atoms (UA), 
    return tuples (i,j) for which atoms connectivity exists, i.e. AC[i,j] == 1.
    """
    bonds = []
...
def AC2BO(AC, atoms, charge, allow_charged_fragments=True, use_graph=True):
    ...
    for i,(atomicNum,valence) in enumerate(zip(atoms,AC_valence)):
        # valence (`atomic_valence`) can't be smaller than number of neighbourgs (`AC_valence` or `valence` below)
        # TODO: This condition is not true in case of coordinate bonding, how will you handle it?
        possible_valence = [x for x in atomic_valence[atomicNum] if x >= valence]
        if not possible_valence:
            print(f'atomicNum: {atomicNum}, possible_valence: {possible_valence}')
            print('Valence of atom',i,'is',valence,'which bigger than allowed max',max(atomic_valence[atomicNum]),'. Stopping')
            sys.exit()
        valences_list_of_lists.append(possible_valence)

    # convert [[4],[2,1]] to [[4,2],[4,1]]
    # or [[4],[2,1], [3,8,9]] to (ignore brackets) (4, 2, 3) (4, 2, 8) (4, 2, 9) (4, 1, 3) (4, 1, 8) (4, 1, 9)
    # i.e. every element of outer list i.e. [4,2...] is valid valency list for all atoms one by one.
